...
def RecoilControl_Update(processHandle, clientBaseAddress, Offsets, Options, ARDUINO_HANDLE):
	...
	try:
		...
		# Читаем указатель на службы отдачи
		aimPunchServicesPtr = memfuncs.ProcMemHandler.ReadPointer(processHandle, localPlayer + Offsets.offset.m_pAimPunchServices)

		if aimPunchServicesPtr:
                        aimPunch_x = memfuncs.ProcMemHandler.ReadFloat(processHandle, aimPunchServicesPtr + Offsets.offset.m_predictableBaseAngle)
                        aimPunch_y = memfuncs.ProcMemHandler.ReadFloat(processHandle, aimPunchServicesPtr + Offsets.offset.m_predictableBaseAngle + 0x4)
		else:
                        aimPunch_x, aimPunch_y = 0.0, 0.0
